...
public class BTree<K extends Comparable<K>, V> implements IBTree<K, V> {
    ...
    private void insertNonFull(IBTreeNode<K, V> r, K key, V value) {
        ...
        int i = size - 1;
        while (i >= 0 && key.compareTo(r.getKeys().get(i)) < 0) i--;
        i++;
        if (r.isLeaf()) {
    ...
    private V search(IBTreeNode<K, V> x, K key) {
        ...
        int i = 0;
        while (i < s && key.compareTo(keys.get(i)) > 0) i++;
    ...
    private boolean delete(IBTreeNode<K, V> x, K key) {
        ...
        while (i < size - 1 && key.compareTo(x.getKeys().get(i)) > 0) i++;
        ...
        if (x.isLeaf() && equal) {
            ...
            IBTreeNode<K, V> leftChild = x.getChildren().get(i);// the child that precedes key
            IBTreeNode<K, V> rightChild = x.getChildren().get(i + 1);// the child that follows key

            if (leftChild.getNumOfKeys() >= this.t) {
                Pair<K, V> predecessor = maximum(leftChild);
                delete(leftChild, predecessor.getKey());
                x.getKeys().remove(i);
                x.getValues().remove(i);
                x.getKeys().add(i, predecessor.getKey());
                x.getValues().add(i, predecessor.getValue());
            } else if (rightChild.getNumOfKeys() >= this.t) {
                Pair<K, V> successor = minimum(rightChild);
                delete(rightChild, successor.getKey());
                x.getKeys().remove(i);
                x.getValues().remove(i);
            ...
            if (target.getNumOfKeys() == t - 1) {
                boolean l = hasLeft(k), r = hasRight(x, k);
    ...
    private Pair<K, V> minimum(IBTreeNode<K, V> x) {
        if (x.isLeaf())
            return new Pair<>(x.getKeys().get(0), x.getValues().get(0));
    ...
    private Pair<K, V> maximum(IBTreeNode<K, V> x) {
        if (x.isLeaf())
            return new Pair<>(x.getKeys().get(x.getNumOfKeys() - 1), x.getValues().get(x.getNumOfKeys() - 1));
